...
/**
 * V2 du questionnaire — intègre la relecture de Max (memo vocal + annotations).
 * Page SÉPARÉE de /audit-questions (V1 conservée intacte). Données isolées :
 * store id = "questions_v2" dans cdv.audit_review.
 *
 * Mêmes fonctions que V1 : accordéon replié, statut à droite, commentaire
 * optionnel, repli auto, badge « Validé », autosave. Ajout : questions chiffrées
 * avec option « je ne sais pas ».
 */

type QType = "text" | "area" | "num" | "choice" | "scale";
type Q = { l: string; t: QType; o?: string[]; ex?: string; g?: string; u?: string; dk?: boolean };
type Section = { tag: string; title: string; desc: string; qs: Q[] };
type Edit = { l?: string; s?: string; c?: string };
type Edits = Record<string, Edit>;

const STORE_ID = "questions_v2";
...
const SECTIONS: Section[] = [
  { tag: "A", title: "Vos coordonnées", desc: "Pour créer votre espace et vous recontacter.", qs: [
  ...
  { tag: "B", title: "Votre activité", desc: "", qs: [
    { l: "Décrivez votre activité : que vendez-vous ?", t: "area", ex: "Je pose des cuisines sur-mesure haut de gamme." },
    { l: "À qui vendez-vous ? Qui sont vos clients ?", t: "area", ex: "Des particuliers aisés, propriétaires, autour de Lyon." },
  ] },
  { tag: "C", title: "Vos chiffres clés", desc: "La data essentielle. Un chiffre précis si vous l'avez — sinon « je ne sais pas », c'est déjà une information (on n'améliore que ce qu'on mesure).", qs: [
    { l: "Objectif de CA sur les 12 prochains mois", t: "num", u: "€", ex: "800 000" },
    { l: "Votre % de croissance de CA sur les 2 dernières années", t: "num", u: "%", ex: "15", dk: true },
    { l: "Votre EBE (excédent brut d'exploitation)", t: "num", u: "€", ex: "120 000", dk: true },
    { l: "Votre marge brute", t: "num", u: "%", ex: "45", dk: true },
    { l: "Nombre de nouveaux prospects générés chaque mois", t: "num", u: "#", ex: "40", dk: true },
    { l: "Nombre de nouveaux clients générés chaque mois", t: "num", u: "#", ex: "8", dk: true },
    { l: "Votre taux de conversion (prospects → clients)", t: "num", u: "%", ex: "20", dk: true },
    ...
    { l: "Mesurez-vous votre flux de trésorerie ? Si oui, quel est-il sur les 12 derniers mois ?", t: "num", u: "€", ex: "90 000", dk: true },
  ...
  { tag: "D", title: "Votre offre", desc: "", qs: [
    ...
    { l: "Entre 0 et 10, à quel point avez-vous une offre irrésistible ?", t: "scale", g: "une offre totalement différente de la concurrence, qui fait que le prospect se sentirait stupide de refuser." },
    { l: "Qu'est-ce qui vous rend vraiment différent de vos concurrents ?", t: "area", ex: "Pose 24 h garantie · SAV internalisé · 15 ans de références." },
  ...
  { tag: "E", title: "Acquisition & vente", desc: "", qs: [
  ...
  { tag: "F", title: "Organisation & pilotage", desc: "", qs: [
    { l: "Quelles sont les tâches qui vous prennent le plus de temps ?", t: "area", ex: "Les devis et le SAV." },
    { l: "Avez-vous des tableaux de bord (commercial, marketing, financier) pour mesurer vos indicateurs clés ? Si oui, lesquels suivez-vous ?", t: "area", ex: "Oui : CA, marge, pipeline commercial." },
  ] },
  { tag: "G", title: "Vous, le chef d'entreprise", desc: "", qs: [
    { l: "Combien d'heures par semaine consacrez-vous à votre entreprise ?", t: "num", u: "#", ex: "60" },
    { l: "Quel est le % de votre temps passé dans l'opérationnel ?", t: "num", u: "%", ex: "70" },
    { l: "Sur 1 à 10, à quel point vous sentez-vous débordé ou seul dans vos décisions ?", t: "scale" },
  ] },
  { tag: "H", title: "Ambition & freins", desc: "", qs: [
    { l: "Que voulez-vous spécifiquement accomplir avec votre entreprise dans les 3 à 5 prochaines années ?", t: "area", ex: "Passer de 500 K à 2 M€, avec une équipe qui gère sans moi." },
    { l: "Quelles sont vos 1 à 3 problématiques majeures ? Vos plus grands freins pour atteindre vos objectifs ?", t: "area", ex: "Tout repose sur moi ; pas de flux régulier de clients." },
  ] },
];
...
export default function ReviewClientV2() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      let html = '<div class="stick"><div class="in"><span class="nm"><span class="d"></span>Diagnostic V2 — relecture</span>' +
      ...
      html += '<div class="wrap"><header>' +
        '<p class="eyebrow">V2 · après retours de Max · à valider</p>' +
        "<h1>Le Diagnostic Business — V2</h1>" +
        '<p class="sub">Version resserrée intégrant tes retours : moins de questions, la data essentielle, l’option « je ne sais pas » sur les chiffres. Objectif : 8-10 min pour le chef d’entreprise.</p>' +
        '<div class="howto"><b>Pour Max :</b> dépliez un bloc, puis agissez sur chaque question — <b>réécrivez</b> le texte, marquez <b class="hl-ok">Validée</b> / <b class="hl-rev">À revoir</b>, ou <b>ajoutez un commentaire</b>. Les réponses affichées sont un <b>aperçu non modifiable</b>. Enregistrement <b>automatique</b> et indépendant de la V1.</div>' +
        '<div class="nav">' +
      ...
      html += '<footer>' + total + ' questions (dont ' + SECTIONS[0].qs.length + ' de coordonnées). Version 2 — après relecture de Max. Rendu d’aperçu, réponses = exemples. Modifs enregistrées automatiquement.</footer></div>' +
