 ...
 */
const STATIC_FORMAT =
  "Structure ta réponse avec ces balises EXACTES, chacune en début de ligne, suivie de son texte, et chacune UNE SEULE FOIS (un seul ##CONSTAT##, un seul ##ACTION##, un seul ##COUT##, un seul ##QUESTION##). " +
  ...
  "(g) VARIE tes formulations pour ne jamais paraître un gabarit : n'OUVRE PAS par « Soyons clairs » (trouve une autre entrée en matière) ; ne termine pas l'action par « vous saurez que ça marche le jour où… » ; dans le coût, NE réécris PAS les mots « taxe stupide » (la section porte déjà ce nom), dis-le autrement (« ce que ça vous coûte », « l'argent qui file »…) ; n'abuse pas de « point barre » / « est-ce que vous me suivez ? » (au plus une fois) ; et change d'images d'une section à l'autre. " +
